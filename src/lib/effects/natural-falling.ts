/**
 * Natural Falling Effect Library - TypeScript / Next.js Port
 * 灵感与物理模型源自: https://github.com/qtqz/natural-falling-effect (Author: qtqz)
 * 经由 Scholarly 论坛架构适配: 100% 严格类型安全、修复 window resize 上下文缺失 bug、内存安全回收
 */

export type FallingEffectType = "petal" | "rain" | "leaf" | "snow";

export interface FallingOptions {
    open?: boolean;
    effectType?: FallingEffectType;
    imgNumSetting?: [number, number, number, number]; // [petal, leaf, snow, rain]
    fadeIn?: boolean;
    fadeOut?: boolean;
    fadeOutTime?: number;
    rainSpeed?: number;
    rainDeviation?: number;
    rainAngle?: number;
    rainHasBounce?: boolean;
    gravity?: number;
    zIndex?: number;
    imgSize?: [number, number, number]; // [petal, leaf, snow]
    windX?: number;
}

export interface FallingInstance {
    destroy: () => void;
}

// 预置内嵌矢量/透明位图资产 (来自开源项目 natural-falling-effect)
const PETAL_BASE64 =
    "data:image/png;base64,";

const LEAF_GINKGO_BASE64 =
    "data:image/png;base64,";

const LEAF_MAPLE_BASE64 =
    "data:image/png;base64,";

interface InternalParticleConfig {
    width: number;
    height: number;
    gravity: number;
    windX: number;
    fadeIn: boolean;
    fadeOut: boolean;
    fadeOutTime: number;
    isTimeOver?: boolean;
    rainSpeed: number;
    rainDeviation: number;
    rainAngle: number;
    rainHasBounce: boolean;
    count: number;
    size: number;
    halfNum?: number;
    images?: {
        petal: HTMLImageElement;
        leaf1: HTMLImageElement;
        leaf2: HTMLImageElement;
    };
}

abstract class BaseParticle {
    x: number;
    y: number;
    size: number;
    config: InternalParticleConfig;

    constructor(x: number, y: number, size: number, config: InternalParticleConfig) {
        this.x = x;
        this.y = y;
        this.size = size;
        this.config = config;
    }

    reset() {
        this.x = Math.random() * this.config.width;
        this.y = -this.size;
    }

    abstract update(): boolean;
    abstract draw(ctx: CanvasRenderingContext2D): void;
}

// 🌸 花瓣粒子 (樱花)
class PetalParticle extends BaseParticle {
    private angle: number;
    private vx = 0;
    private vy = 0;
    private va = 0;

    constructor(x: number, y: number, size: number, config: InternalParticleConfig) {
        super(x, y, size, config);
        this.angle = Math.random() * 6;
        this.updateVelocity();
        this.size = Math.floor(this.size * (Math.random() * 0.75 + 0.25));
        if (this.config.fadeIn) {
            this.y -= Math.random() * 500;
        }
    }

    updateVelocity() {
        this.vx = this.config.windX ? this.config.windX / 40 : 2.5 * (Math.random() - 0.5);
        this.vy = 1.2 + Math.random() * this.config.gravity * 5;
        this.va = Math.random() * 0.03;
    }

    update(): boolean {
        this.x += this.vx;
        this.y += this.vy;
        this.angle += this.va;

        if (this.y > this.config.height) {
            if (this.config.isTimeOver) return false;
            this.reset();
        }

        if (this.x > this.config.width) {
            this.x = 0;
        } else if (this.x < 0) {
            this.x = this.config.width;
        }

        return true;
    }

    draw(ctx: CanvasRenderingContext2D) {
        if (!this.config.images?.petal) return;
        ctx.save();
        ctx.translate(this.x, this.y);
        ctx.rotate(this.angle);
        ctx.drawImage(this.config.images.petal, 0, 0, this.size, this.size);
        ctx.restore();
    }
}

// 🍂 树叶粒子 (银杏与枫叶)
class LeafParticle extends BaseParticle {
    private angle: number;
    private vx = 0;
    private vy = 0;
    private va = 0;
    private img: HTMLImageElement | null = null;

    constructor(x: number, y: number, size: number, config: InternalParticleConfig, index: number) {
        super(x, y, size, config);
        this.angle = Math.random() * 6;
        this.updateVelocity();
        this.size = Math.floor(this.size * (Math.random() * 0.7 + 0.3));
        if (this.config.images) {
            const half = this.config.halfNum ?? Math.floor(this.config.count / 2);
            this.img = index < half ? this.config.images.leaf1 : this.config.images.leaf2;
        }
    }

    updateVelocity() {
        this.vx = this.config.windX ? this.config.windX / 40 : 3 * (Math.random() - 0.5);
        this.vy = 1.5 + Math.random() * this.config.gravity * 5;
        this.va = Math.random() * 0.03;
    }

    update(): boolean {
        this.x += this.vx;
        this.y += this.vy;
        this.angle += this.va;

        if (this.y > this.config.height) {
            if (this.config.isTimeOver) return false;
            this.reset();
        }

        if (this.x > this.config.width) {
            this.x = 0;
        } else if (this.x < 0) {
            this.x = this.config.width;
        }

        return true;
    }

    draw(ctx: CanvasRenderingContext2D) {
        if (!this.img) return;
        ctx.save();
        ctx.translate(this.x, this.y);
        ctx.rotate(this.angle);
        ctx.drawImage(this.img, 0, 0, this.size, this.size);
        ctx.restore();
    }
}

// ❄️ 雪花粒子
class SnowParticle extends BaseParticle {
    private opacity: number;
    private vx = 0;
    private vy = 0;

    constructor(x: number, y: number, size: number, config: InternalParticleConfig) {
        super(x, y, size, config);
        this.opacity = Math.random() * 0.5 + 0.5;
        if (this.opacity > 0.8) this.opacity = 1;
        this.updateVelocity();
        this.size = Math.random() * this.size + 2;
    }

    updateVelocity() {
        this.vx = this.config.windX ? this.config.windX / 40 : 2 * (Math.random() - 0.5);
        this.vy = 0.8 + Math.random() * this.config.gravity * 4;
    }

    update(): boolean {
        this.x += this.vx;
        this.y += this.vy;

        if (this.y > this.config.height) {
            if (this.config.isTimeOver) return false;
            this.reset();
        }

        if (this.x > this.config.width) {
            this.x = 0;
        } else if (this.x < 0) {
            this.x = this.config.width;
        }

        return true;
    }

    draw(ctx: CanvasRenderingContext2D) {
        ctx.save();
        ctx.globalAlpha = this.opacity;
        ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
        ctx.beginPath();
        ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2, false);
        ctx.fill();
        ctx.restore();
    }
}

// 💧 雨滴飞溅微水花
class BounceParticle {
    private x: number;
    private y: number;
    private config: InternalParticleConfig;
    private radius: number;
    private vx: number;
    private vy: number;

    constructor(x: number, y: number, config: InternalParticleConfig) {
        this.x = x;
        this.y = y;
        this.config = config;
        this.radius = 0.5 + Math.random() * 0.8;
        const dist = Math.random() * Math.max(config.rainSpeed / 12, 3);
        const angle = Math.PI + Math.random() * Math.PI;
        this.vx = Math.cos(angle) * dist;
        this.vy = Math.sin(angle) * dist;
    }

    update(): boolean {
        this.vy += this.config.gravity;
        this.x += this.vx;
        this.y += this.vy;
        return this.y <= this.config.height;
    }

    draw(ctx: CanvasRenderingContext2D) {
        ctx.fillStyle = "rgba(223, 223, 223, 0.6)";
        ctx.beginPath();
        ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2);
        ctx.fill();
    }
}

// 🌧️ 雨滴粒子
class RainParticle extends BaseParticle {
    private bounces: BounceParticle[] = [];
    private speed = 0;
    private vx = 0;
    private vy = 0;
    private px = 0;
    private py = 0;

    constructor(x: number, y: number, size: number, config: InternalParticleConfig) {
        super(x, y, 2, config);
        this.updateVelocity();
    }

    updateVelocity() {
        const angleRad = this.config.rainAngle * 0.017;
        this.speed = 20 * (Math.random() - 0.5) + this.config.rainSpeed;
        this.vx = this.speed * Math.cos(angleRad) + (Math.random() - 0.5) * this.config.rainDeviation;
        this.vy = -this.speed * Math.sin(angleRad);
        this.px = this.x;
        this.py = this.y;
    }

    update(): boolean {
        this.px = this.x;
        this.py = this.y;
        this.vy += this.config.gravity;
        this.x += this.vx;
        this.y += this.vy;

        this.bounces = this.bounces.filter((bounce) => bounce.update());

        if (this.y > this.config.height) {
            if (this.config.isTimeOver && Math.random() > 0.93) return false;

            if (this.config.rainHasBounce) {
                const bounceCount = Math.round(2 + Math.random() * 3);
                for (let i = 0; i < bounceCount; i++) {
                    this.bounces.push(new BounceParticle(this.x, this.config.height, this.config));
                }
            }

            this.reset();
            this.px = this.x;
            this.py = this.y;
            this.vy = -this.speed * Math.sin(this.config.rainAngle * 0.017);
        }

        if (this.x > this.config.width) {
            this.x = 0;
            this.px = this.x;
        } else if (this.x < 0) {
            this.x = this.config.width;
            this.px = this.x;
        }

        return true;
    }

    draw(ctx: CanvasRenderingContext2D) {
        const gradient = ctx.createLinearGradient(this.px, this.py, this.x, this.y);
        gradient.addColorStop(0, "rgba(255,255,255,0)");
        gradient.addColorStop(0.5, "rgba(200,225,255,0.65)");

        ctx.strokeStyle = gradient;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(this.px, this.py);
        ctx.lineTo(this.x, this.y);
        ctx.stroke();

        this.bounces.forEach((bounce) => bounce.draw(ctx));
    }
}

/**
 * 核心渲染控制器
 */
class SingleFallingRunner {
    private type: FallingEffectType;
    private config: InternalParticleConfig;
    private canvas: HTMLCanvasElement | null = null;
    private ctx: CanvasRenderingContext2D | null = null;
    private particles: BaseParticle[] = [];
    private destroyed = false;
    private animationId: number | null = null;
    private lastTimestamp = 0;
    private frameInterval = 1000 / 60;

    constructor(type: FallingEffectType, config: InternalParticleConfig) {
        this.type = type;
        this.config = config;

        this.handleResize = this.handleResize.bind(this);
        this.animate = this.animate.bind(this);

        this.initCanvas();
        this.initParticles();

        if (this.config.fadeOut && this.config.fadeOutTime >= 1) {
            setTimeout(() => {
                this.config.isTimeOver = true;
            }, this.config.fadeOutTime * 1000);

            setTimeout(() => {
                this.destroy();
            }, (this.config.fadeOutTime + 15) * 1000);
        }

        this.animationId = requestAnimationFrame(this.animate);
    }

    private handleResize() {
        if (!this.canvas || !this.ctx) return;
        this.config.width = window.innerWidth;
        this.config.height = window.innerHeight;
        this.canvas.width = window.innerWidth;
        this.canvas.height = window.innerHeight;
    }

    private initCanvas() {
        this.canvas = document.createElement("canvas");
        this.ctx = this.canvas.getContext("2d");

        this.handleResize();

        // 样式保障：完全置底微浮动、零点击拦截
        this.canvas.style.position = "fixed";
        this.canvas.style.left = "0";
        this.canvas.style.top = "0";
        this.canvas.style.width = "100vw";
        this.canvas.style.height = "100vh";
        this.canvas.style.pointerEvents = "none";
        this.canvas.style.zIndex = "12"; // 优雅位于背景之后或微高于背景层，保证内容与交互清爽

        document.body.appendChild(this.canvas);
        window.addEventListener("resize", this.handleResize);
    }

    private initParticles() {
        this.particles = [];
        const count = this.config.count;

        if (this.type === "leaf") {
            this.config.halfNum = Math.floor(count / 2);
        }

        for (let i = 0; i < count; i++) {
            const x = Math.random() * this.config.width;
            const y = this.config.fadeIn
                ? Math.random() * this.config.height - this.config.height
                : Math.random() * this.config.height;

            let particle: BaseParticle;
            if (this.type === "petal") {
                particle = new PetalParticle(x, y, this.config.size, this.config);
            } else if (this.type === "leaf") {
                particle = new LeafParticle(x, y, this.config.size, this.config, i);
            } else if (this.type === "snow") {
                particle = new SnowParticle(x, y, this.config.size, this.config);
            } else {
                particle = new RainParticle(x, y, this.config.size, this.config);
            }

            this.particles.push(particle);
        }
    }

    private animate(timestamp: number) {
        if (this.destroyed || !this.ctx || !this.canvas) return;

        const delta = timestamp - this.lastTimestamp;
        if (delta >= this.frameInterval) {
            this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);

            this.particles = this.particles.filter((p) => {
                if (p.update()) {
                    if (this.ctx) p.draw(this.ctx);
                    return true;
                }
                return false;
            });

            this.lastTimestamp = timestamp;
        }

        this.animationId = requestAnimationFrame(this.animate);
    }

    destroy() {
        this.destroyed = true;
        if (this.animationId !== null) {
            cancelAnimationFrame(this.animationId);
            this.animationId = null;
        }
        window.removeEventListener("resize", this.handleResize);
        if (this.canvas && this.canvas.parentNode) {
            this.canvas.parentNode.removeChild(this.canvas);
        }
        this.particles = [];
        this.canvas = null;
        this.ctx = null;
    }
}

/**
 * 预加载图片资源缓存（单例）
 */
let cachedImages: {
    petal: HTMLImageElement;
    leaf1: HTMLImageElement;
    leaf2: HTMLImageElement;
} | null = null;

function getSharedImages() {
    if (cachedImages) return cachedImages;
    if (typeof window === "undefined") return null;

    const petal = new Image();
    petal.src = PETAL_BASE64;

    const leaf1 = new Image();
    leaf1.src = LEAF_GINKGO_BASE64;

    const leaf2 = new Image();
    leaf2.src = LEAF_MAPLE_BASE64;

    cachedImages = { petal, leaf1, leaf2 };
    return cachedImages;
}

/**
 * 创建并运行自然飘落特效
 * 默认关闭，调用时可传入所选的季节特效类型
 */
export function startFallingEffect(options: FallingOptions = {}): FallingInstance {
    if (typeof window === "undefined" || !options.open) {
        return { destroy: () => {} };
    }

    const effectType: FallingEffectType = options.effectType || "petal";
    const images = getSharedImages();

    const counts: Record<FallingEffectType, number> = {
        petal: options.imgNumSetting?.[0] ?? 36,
        leaf: options.imgNumSetting?.[1] ?? 32,
        snow: options.imgNumSetting?.[2] ?? 65,
        rain: options.imgNumSetting?.[3] ?? 50,
    };

    const sizes: Record<FallingEffectType, number> = {
        petal: options.imgSize?.[0] ?? 38,
        leaf: options.imgSize?.[1] ?? 36,
        snow: options.imgSize?.[2] ?? 2.4,
        rain: 2,
    };

    const config: InternalParticleConfig = {
        width: window.innerWidth,
        height: window.innerHeight,
        gravity: options.gravity ?? 0.16,
        windX: options.windX ?? 0,
        fadeIn: options.fadeIn ?? true,
        fadeOut: options.fadeOut ?? false,
        fadeOutTime: options.fadeOutTime ?? 20,
        rainSpeed: options.rainSpeed ?? 45,
        rainDeviation: options.rainDeviation ?? 4,
        rainAngle: options.rainAngle ?? 265,
        rainHasBounce: options.rainHasBounce ?? true,
        count: counts[effectType],
        size: sizes[effectType],
        images: images || undefined,
    };

    const runner = new SingleFallingRunner(effectType, config);

    return {
        destroy: () => runner.destroy(),
    };
}
